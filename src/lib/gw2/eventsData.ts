...
// Server-side loader for the GW2 wiki event-timer schedule. Shared by the
// /api/events route (client timeline) and the per-meta pages (SSR/ISR), so both
// use the same weekly-cached fetch.
...
/** Convenience: the single located meta event by id, or null. */
export async function getMetaEvent(id: string): Promise<MetaEvent | null> {
  const events = await getEvents().catch(() => [] as MetaEvent[]);
